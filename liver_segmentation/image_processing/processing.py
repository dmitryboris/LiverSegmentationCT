...
def process_image(image_path):
    ...
    image = next(data_g)
    predicted_mask = model.predict(image).astype('uint8').squeeze()
    ...
    overlay_url = os.path.join(settings.MEDIA_URL, 'result', f"{name}.png")
    return overlay_url
